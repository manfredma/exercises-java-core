...
import static manfred.exercises.lang.stream.model.Dish.dishTags;
import java.util.*;
...
public class Grouping {
    ...
    private static Map<Dish.Type, Set<String>> groupDishTagsByType() {
        return menu.stream().collect(groupingBy(Dish::getType, mapping(dish -> dishTags.get(dish.getName()).stream(), collectingAndThen(toSet(), a -> a.stream().flatMap(stringStream -> stringStream).collect(toSet())))));
    }
