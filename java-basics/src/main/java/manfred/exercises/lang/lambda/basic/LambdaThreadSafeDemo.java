package manfred.exercises.lang.lambda.basic;
...
/**
 * 对比串行流、并行流与 CompletableFuture 三种并发处理方式的执行效果。
 *
 * 通过对水果列表的模拟耗时处理，直观展示串行流的顺序执行、
 * parallelStream 利用 ForkJoinPool 并发执行、
 * 以及 CompletableFuture 配合自定义线程池并行执行的线程调度差异，
 * 同时包含串行流与并行流对大规模求和操作的性能对比。
 */
public class LambdaThreadSafeDemo {
